...
class Client extends EventEmitter {
    ...
    constructor(options = {}) {
         ...
         * @private
         */
        this._options = Object.assign(CLIENT_OPTIONS, options);

        /**
         * @private
         */
        this._trace = this._options.debug ? console.debug : () => {
        };
    ...
    connect() {
        try {
            this._socket.bind(() => {
                ...
                this._trace('SOCKET', 'Connected to device at %s', this._options.host);
            });
        } catch (err) {
            const message = 'Unable to connect (' + err.message + '). Retrying in ' + this._options.connectTimeout + 'ms...';
            this._trace('SOCKET', message);
            this.emit('error', message, err);

            setTimeout(() => this.connect(), this._options.connectTimeout);
        }
    }
    ...
    _handleResponse(buffer) {
        ...
        if (pack.t === 'dev') {
            return this._handleHandshakeResponse(message);
        }
        ...
        this._trace('IN.MSG', 'Unknown message of type %s: %s, %s', pack.t, message, pack)
    }
    ...
    _handleBindingConfirmationResponse(pack) {
        ...
        if (this._options.poll) {
            this._intervalRef = setInterval(() => this._requestStatus(), this._options.pollingInterval);
        }
